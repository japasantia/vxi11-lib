...
extern "C" int	vxi11_open_device(const char *ip, CLINK *clink);

int	vxi11_open_device(const char *ip, CLINK *clink, char *device);

extern "C" int	vxi11_close_device(const char *ip, CLINK *clink);

int	vxi11_send(CLINK *clink, const char *cmd);

extern "C" int	vxi11_send(CLINK *clink, const char *cmd, unsigned long len);

long	vxi11_receive(CLINK *clink, char *buffer, unsigned long len);

extern "C" long	vxi11_receive(CLINK *clink, char *buffer, unsigned long len, unsigned long timeout);

/* Utility functions, that use send() and receive(). Use these too. */

extern "C" int	vxi11_send_data_block(CLINK *clink, const char *cmd, char *buffer, unsigned long len);

extern "C" long	vxi11_receive_data_block(CLINK *clink, char *buffer, unsigned long len, unsigned long timeout);

extern "C" long	vxi11_send_and_receive(CLINK *clink, const char *cmd, char *buf, unsigned long buf_len, unsigned long timeout);

extern "C" long	vxi11_obtain_long_value(CLINK *clink, const char *cmd, unsigned long timeout);

extern "C" double vxi11_obtain_double_value(CLINK *clink, const char *cmd, unsigned long timeout);
